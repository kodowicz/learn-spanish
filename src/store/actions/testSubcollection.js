...
export const basicTwoTerms = number => (dispatch, getState, { getFirestore }) => {
  ...
  const docRef = firestore.collection("subcollection/7Wlu4hMX2ILQPLHHZ9Js/sets");
...
export const removeTerm = termID => (dispatch, getState, { getFirestore }) => {
  ...
  const docRef = firestore.doc(`subcollection/7Wlu4hMX2ILQPLHHZ9Js/sets/${termID}`);
...
export const addNewTerm = () => (dispatch, getState, { getFirestore }) => {
  const firestore = getFirestore();
  const authId = getState().firebase.auth.uid;
  const newDocument = firestore.collection("subcollection/7Wlu4hMX2ILQPLHHZ9Js/sets").doc();
